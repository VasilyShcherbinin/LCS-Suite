...
def randomize():
    with open("../XCS/Demo_Datasets/" + str(bits) + "Multiplexer_Data_Complete.txt", 'r') as source:
        data = [(random.random(), line) for line in source]
    data[1:] = sorted(data[1:])
    with open("../XCS/Demo_Datasets/" + str(bits) + "Multiplexer_Data_Complete_Randomized.txt", 'w') as target:
        for _, line in data:
            target.write(line)
