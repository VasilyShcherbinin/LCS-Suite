...
def randomize():
    with open("../UCS/Demo_Datasets/"+str(bits)+"Parity_Data_Complete.txt", 'r') as source:
        data = [(random.random(), line) for line in source]
    data[1:] = sorted(data[1:])
    with open("../UCS/Demo_Datasets/"+str(bits)+"Parity_Data_Complete_Randomized.txt", 'w') as target:
        for _, line in data:
            target.write(line)
